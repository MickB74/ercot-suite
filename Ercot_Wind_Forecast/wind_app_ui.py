...
def _render_sidebar(st, wiring: Wiring):
    ...
    if mode.startswith("Historical"):
        yr = st.selectbox("Year", [str(y) for y in range(tzutil.now_central().year, 2014, -1)],
                          index=1, key="wf_year")
    ...
    if compare_year and mode.startswith("Historical"):
        yr2 = st.selectbox("Compare to year", [str(y) for y in range(tzutil.now_central().year, 2014, -1)],
...
def _render_sced_compare(st, go, wiring: Wiring, lat: float, lon: float,
    ...
    with st.expander("⚖️ Compare to actual ERCOT SCED generation", expanded=False):
        ...
        guess = tzutil.now_central().year - 1
        for lbl in results:
        ...
        cyear = c1.number_input("SCED calendar year", min_value=2018,
                                max_value=tzutil.now_central().year,
                                value=min(max(guess, 2018), tzutil.now_central().year),
