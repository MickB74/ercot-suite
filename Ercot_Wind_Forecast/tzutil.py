"""Canonical US/Central timezone helpers (DST-correct) for this ERCOT repo.

Standalone copy of the ERCOT Data Hub's ``ercot_core.tz`` (kept in sync by hand).
ERCOT settles in Central Prevailing Time; the November fall-back hour repeats, so
naive wall-clock arithmetic is unsafe for settlement / invoice joins. Lift naive
Central labels to tz-aware Central (DST-handled) before any groupby/dedupe/merge,
and anchor "today"/lag logic on Central, not the machine's local clock.
"""
