...
- **Projected Bill** — a forward *estimate* (clearly labelled as such, not an
  ...
  typical-meteorological-year shape for Mesquite Star's coordinates/system, rescaled to
...
- The `Ercot_Data_Hub` repo present as a sibling directory (or point
  `MESQUITE_STAR_HUB_ROOT` at it). That repo owns the engine and the Mesquite Star data it
...
# or just double-click "Open Mesquite Star Portal.command"
```
...
collect each month. Top up Mesquite Star's generation **and** node prices to the
