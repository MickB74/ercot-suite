...
st.success(
    f"Over **{cov_min} → {cov_max}**, Mesquite Star produced **{mwh:,.0f} MWh**, captured at "
...
if len(monthly) > 1:
    ...
    if download_block_m is not None:
        download_block_m(
            ...
            title=f"Mesquite Star — monthly settlement {start_d} → {end_d}",
...
if recon is None:
    with st.expander("Cross-check vs EIA-923 (independent source)"):
        st.caption(
            ...
            "looks off. Mesquite Star isn't mapped to an EIA plant id yet (there's no "
...
if download_block is not None:
    download_block(
        ...
        title=f"Mesquite Star settlement — {start_d} → {end_d}",
        meta={"Asset": a["project_name"], "Settles at": loc,
              "Structure": terms["structure"], "Strike": f"${terms['strike']:,.2f}/MWh",
