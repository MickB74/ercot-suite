#!/usr/bin/env python3
"""Top up Mesquite Star's cached data to the latest available ERCOT date — one command.
...
Advances both streams the portal settles on, into the shared Data Hub lake:
  ...
  * node price — RT15 settlement-point price at WH_WIND_ALL
...
Run it in the **Hub's** virtualenv (it has gridstatus + the ERCOT credentials):
    ...
    # or double-click "Refresh Mesquite Star Data.command"
...
def main() -> int:
    ...
    print(f"Mesquite Star ({node}) — latest available SCED date: {latest}\n")
