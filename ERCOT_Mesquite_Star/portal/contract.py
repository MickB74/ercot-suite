...
# ── the asset ───────────────────────────────────────────────────────────────
# Mesquite Star (aka Whitehorse Wind, ERCOT queue 19INR0080) — 418.9 MW wind,
# Fisher County. ERCOT aggregate node WH_WIND_ALL (units WH_WIND_UNIT1/UNIT2).
# Owner: Clearway Energy Group. Offtakers: Intuit, Ecolab, Lowe's, Brown University.
ASSET = {
    "project_name": "Mesquite Star",
    "resource_node": "WH_WIND_ALL",
    "resource_name": "WH_WIND_UNIT1",
    # Both SCED units make up the 418.9 MW plant; settlement must sum BOTH,
    # not just resource_name, or it counts ~half the plant.
    "sced_units": ["WH_WIND_UNIT1", "WH_WIND_UNIT2"],
    "capacity_mw": 418.9,
    "tech": "Wind",
    "tracking_type": None,
    "hub": "HB_WEST",
    ...
    "lon": -100.569,
    "dc_ac_ratio": None,
    # EIA-923 plant identifier for the independent generation cross-check.
    # Mesquite Star = EIA plant 62587 (418.9 MW, Fisher County). Overridable via
    # "eia_plant_id" in config.json; None ⇒ the cross-check is disabled.
    "eia_plant_id": 62587,
    "eia_prime_mover": None,   # all prime movers at the plant
    "turbine_model": "SG-3.465-132",
    "turbine_manuf": "Siemens Gamesa Renewable Energy",
    "hub_height_m": 101.5,
...
DEFAULT_CONTRACT = {
    ...
    "settle_point": "WH_WIND_ALL",  # RTM SPP at the resource node
    ...
    "offtaker": "Aggregated; Brown University",              # full company name of the VPPA buyer
    "developer": "Clearway Energy Group",             # developer / entity above the project SPV
    "currency": "USD",
...
def settle_location(terms: dict) -> str:
    ...
    ``WH_WIND_ALL`` or any trading hub such as ``HB_SOUTH``). When it's blank,
...
def eia_plant_id() -> int | None:
    """Mesquite Star's EIA-923 plant id for the SCED cross-check, or None if unmapped.
