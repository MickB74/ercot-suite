"""Locate the shared ERCOT Data Hub and read Mesquite Star's cached data.
...
and cached for the Mesquite Star node. No live ERCOT fetch happens here, so the portal
loads instantly and works offline.

Resolution order for the Hub root (first that exists wins):
  1. ``$MESQUITE_STAR_HUB_ROOT`` environment variable
...
def _candidate_roots() -> list[Path]:
    roots: list[Path] = []
    env = os.environ.get("MESQUITE_STAR_HUB_ROOT")
    if env:
...
def hub_root() -> Path:
    ...
    raise FileNotFoundError(
        ...
        "and the cached Mesquite Star data). Tried:\n  " + tried +
        "\n\nKeep `Ercot_Data_Hub` as a sibling of this repo, or set the "
        "MESQUITE_STAR_HUB_ROOT environment variable to its path."
    )
...
# --------------------------------------------------------------------------- #
# Data-lake readers — Mesquite Star node generation & node prices (RT15)
# --------------------------------------------------------------------------- #
...
def available_locations() -> tuple[str, ...]:
    ...
    The node (``WH_WIND_ALL``) always leads; the trading hubs are those with
...
def wind_typical_hourly() -> pd.DataFrame | None:
    ...
    The Hub's ``plant_value`` step has run the ERA5/USWTDB wind model for Mesquite
    Star's coordinates and cached an 8,760-hour AC profile at
    ``data/plant_value/windgen_WH_WIND_ALL_{year}_{cap}mw.parquet`` (most-recent
    file wins). Returns an hourly frame with an ``ac_kw`` column, or None.
    """
    core()
    from ercot_core import paths  # noqa: PLC0415
    matches = sorted(paths.PLANT_VALUE_DIR.glob("windgen_WH_WIND_ALL_*mw.parquet"))
    if not matches:
        return None
...
def solar_tmy_hourly(resource_name: str, capacity_kw: float) -> pd.DataFrame | None:
    ...
    Meteorological-Year weather for Mesquite Star's coordinates/system and cached the
