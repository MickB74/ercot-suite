"""Run the shared settlement engine for Mesquite Star over a date window.
...
def settle(start_date: dt.date, end_date: dt.date, terms: dict | None = None) -> dict | None:
    """Settle Mesquite Star over [start_date, end_date] inclusive. None if no data."""
    ...
    ref = contract.settle_location(terms)         # node WH_WIND_ALL or a trading hub
...
def tmy_monthly_mwh(share: float = 1.0) -> pd.Series | None:
    """Typical-year MWh by calendar month (1–12) from the cached wind-model run.

    The "expected" production shape for a representative weather year at Mesquite Star's
    own coordinates and turbine fleet, scaled to the offtaker's ``share`` of the
    plant. Returns a Series indexed 1–12, or None if no wind profile is cached.
    """
    hourly = hub.wind_typical_hourly()
    if hourly is None or hourly.empty:
        return None
    ac_kw = hourly["ac_kw"]
    mwh = ac_kw.groupby(ac_kw.index.month).sum() / 1000.0  # hourly kW → MWh
