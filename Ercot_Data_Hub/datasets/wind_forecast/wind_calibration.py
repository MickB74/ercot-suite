...
def region_bias_multiplier(lat=None, lon=None, hub_name=None, table=None) -> tuple[float, str]:
    ...
    if hub and hub in hub_map:
        ...
        # systematically under-predict ERCOT wind, so the fitted hub priors run
        # ~1.5–1.9 (re-derived 2026-06 against metered SCED and the retuned curves).
...
def calibrate_against_actuals(modeled: pd.Series, actual: pd.Series,
    ...
    # Align timezones before the index join: modeled is tz-aware Central, but
    # uploaded actuals may be naive or in another zone (else overlap collapses).
    from ercot_core import tz
    if isinstance(mod.index, pd.DatetimeIndex):
        mod.index = tz.localize_central(mod.index)
    if isinstance(act.index, pd.DatetimeIndex):
        act.index = tz.localize_central(act.index)
