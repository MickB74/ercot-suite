...
from ercot_core.gridstatus_client import ercot as _ercot
...
def build_catalog(date: str | None = None, with_types: bool = False) -> pd.DataFrame:
    ...
    if with_types:
        ...
        print(f"[catalog] enriching with SCED resource types ({sced_day}, via shared cache)...")
        try:
            # Shared 60-day SCED cache (de-duplicated with plant_sced).
            gen = sced_disclosure.get_daily_disclosure(sced_day)
            types = (gen[["resource_name", "resource_type"]]
                     .drop_duplicates("resource_name")
                     .set_index("resource_name")["resource_type"])
