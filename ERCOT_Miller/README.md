...
- **Projected Bill** — a forward *estimate* (clearly labelled as such, not an
  ...
  typical-meteorological-year shape for Miller's coordinates/system, rescaled to
...
- The `Ercot_Data_Hub` repo present as a sibling directory (or point
  `MILLER_HUB_ROOT` at it). That repo owns the engine and the Miller data it
...
# or just double-click "Open Miller Portal.command"
```
...
collect each month. Top up Miller's generation **and** node prices to the
