...
def run_pvwatts(weather: WeatherResult, system: SystemConfig) -> pd.DataFrame:
    ...
    idx = df.index
    # Guarantee a tz-aware Central index. The ERA5 path is already Central, but
    # the NSRDB/PSM4 path can come back naive — localize it so the output (and
    # any join to Central-time ERCOT generation) is unambiguous.
    if isinstance(idx, pd.DatetimeIndex):
        import tzutil
        idx = tzutil.localize_central(idx)
