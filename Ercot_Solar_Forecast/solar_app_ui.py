...
def _render_sced_compare(st, go, wiring: Wiring, project: dict, results: dict, year) -> None:
    ...
    with st.expander("⚖️ Compare to actual ERCOT SCED generation", expanded=False):
        ...
        if kind == "year":
            ...
            yr_guess = tzutil.now_central().year - 1     # TMY → default to last full year
        default_year = min(max(yr_guess, 2018), tzutil.now_central().year)
        c1, c2 = st.columns([1, 2])
        cyear = c1.number_input("SCED calendar year", min_value=2018,
                                max_value=tzutil.now_central().year, value=default_year, step=1,
                                key="sf_sced_year")
