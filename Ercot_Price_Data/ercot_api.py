...
import tzutil  # canonical US/Central helpers (DST-correct); ERCOT operating day
...
PARQUET_PATH = DATA_DIR / "ercot_hub_prices_15min.parquet"
CSV_PATH = DATA_DIR / "ercot_hub_prices_15min.csv"
STATE_PATH = DATA_DIR / ".last_update.json"
...
# Every ERCOT trading hub settlement point.
HUBS = [
    "HB_BUSAVG",   # bus average
    "HB_HUBAVG",   # average of trading hubs
    "HB_HOUSTON",
    "HB_NORTH",
    "HB_PAN",      # Panhandle
    "HB_SOUTH",
...
DEFAULT_BACKFILL_START = date(tzutil.now_central().year - 1, 1, 1).isoformat()
...
def save_config(cfg: dict) -> None:
    ...
    try:
        os.chmod(CONFIG_PATH, 0o600)  # password lives here -- keep it private
    except OSError:
        pass
...
def have_credentials(cfg: dict | None = None) -> bool:
    ...
    return all(cfg.get(k) for k in ("username", "password", "subscription_key"))
...
def archive_backfill(tokens, key, start: date, end: date, store: pd.DataFrame,
    ...
    for mi, (m_start, m_end) in enumerate(months, 1):
        ...
        if month_frames:
            ...
            store = store.drop_duplicates(subset=_dedup_keys(store), keep="last")
            save_store(store, write_csv=False)  # checkpoint (parquet only; CSV at end)
            write_state({"last_success": tzutil.now_central().isoformat(),
...
def _dedup_keys(df: pd.DataFrame) -> list[str]:
    """Store identity key. Includes dst_flag so BOTH passes of the duplicated
    fall-back hour are kept (else the November DST day stores 96 not 100)."""
...
def days_since_update() -> float | None:
    ...
    try:
        ...
        return None
    # Compare in Central; tolerate older naive-local state files.
    now = tzutil.now_central()
    if last.tz is None:
        last = last.tz_localize(tzutil.CENTRAL, ambiguous=True, nonexistent="shift_forward")
...
def update(progress_callback=None) -> dict:
    ...
    write_state({"last_success": tzutil.now_central().isoformat(), **summary})
