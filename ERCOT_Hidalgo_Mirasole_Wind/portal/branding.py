...
def hero(st, title: str, subtitle: str = "") -> None:
    ...
    chips = "".join(
        f'<span class="portal-chip">{c}</span>' for c in (
    ...
    st.markdown(
        ...
        f'<div class="eyebrow">Hidalgo Mirasole Wind · Settlement Portal</div>'
...
def footer(st) -> None:
    year = pd_year()
    st.markdown(
        f'<div class="portal-foot">Hidalgo Mirasole Wind Settlement Portal · prepared by '
