"""Run the shared settlement engine for Hidalgo Mirasole Wind over a date window.
...
def settle(start_date: dt.date, end_date: dt.date, terms: dict | None = None) -> dict | None:
    """Settle Hidalgo Mirasole Wind over [start_date, end_date] inclusive. None if no data."""
    ...
    ref = contract.settle_location(terms)         # node MIRASOLE_GEN or a trading hub
...
def tmy_monthly_mwh(share: float = 1.0) -> pd.Series | None:
    """Typical-year MWh by calendar month (1–12) from the cached PVWatts TMY run.

    The "expected" production shape for a representative weather year at Hidalgo Mirasole Wind's
    own coordinates and system config, scaled to the offtaker's ``share`` of the
