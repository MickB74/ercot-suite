...
- **Projected Bill** — a forward *estimate* (clearly labelled as such, not an
  ...
  typical-meteorological-year shape for Hidalgo Mirasole Wind's coordinates/system, rescaled to
...
- The `Ercot_Data_Hub` repo present as a sibling directory (or point
  `HIDALGO_MIRASOLE_WIND_HUB_ROOT` at it). That repo owns the engine and the Hidalgo Mirasole Wind data it
...
# or just double-click "Open Hidalgo Mirasole Wind Portal.command"
```
...
collect each month. Top up Hidalgo Mirasole Wind's generation **and** node prices to the
