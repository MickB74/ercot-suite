...
with tab_long:
    ...
    if tmy_mwh is None:
        ...
        st.sidebar.caption(
            "Generation is projected from the historical metered shape. "
            "Run the Hub's plant-value step to enable the calibrated model.")
    basis = st.sidebar.radio(
        "Generation basis", bases, index=0,
        help="**Calibrated model** — typical-year shape rescaled to match "
             "Hidalgo Mirasole Wind's real metered output. "
             "**Physical model** — raw TMY, uncalibrated. "
             "**Historical shape** — mean of each calendar month.")
    ...
    if basis == "Calibrated model" and cal:
        ...
        st.caption(f"**Physical model** — raw typical year "
