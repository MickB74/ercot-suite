...
from portal import analytics, branding, contract, hub, settings, statement  # noqa: E402
...
with st.expander(hdr, expanded=folder is None and not cur):
    new = st.text_input(
        ...
        placeholder=str(Path.home() / "Hidalgo Mirasole Wind" / "statements"),
...
def _run_batch(items):
    ...
    dl = hub.export_block()
    if dl is not None:
        dl(st, summ, name="portal_portfolio_audit",
           title="Hidalgo Mirasole Wind — portfolio invoice audit",
...
if net_mode:
    ...
    if download_block is not None:
        download_block(
            st, intervals, name=f"portal_net_settlement_audit_{pd.Timestamp(lo).date()}",
            title="Hidalgo Mirasole Wind — net settlement audit",
        ...
        st.download_button("⬇ Download net-settlement audit CSV",
                           ...
                           file_name="portal_net_settlement_audit.csv", mime="text/csv")
    branding.footer(st)
    st.stop()
...
if download_block is not None:
    download_block(
        st, intervals, name=f"portal_invoice_audit_{pd.Timestamp(lo).date()}",
        title="Hidalgo Mirasole Wind — invoice audit",
        meta={"Asset": a["project_name"], "Settles at": loc, "Statement": src_label or "",
    ...
    st.download_button("⬇ Download reconciliation CSV",
                       intervals.to_csv(index=False).encode("utf-8"),
                       file_name="portal_invoice_audit.csv", mime="text/csv")
