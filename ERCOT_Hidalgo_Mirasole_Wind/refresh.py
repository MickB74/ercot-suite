#!/usr/bin/env python3
"""Top up Hidalgo Mirasole Wind's cached data to the latest available ERCOT date — one command.
...
Advances both streams the portal settles on, into the shared Data Hub lake:
  ...
  * node price — RT15 settlement-point price at MIRASOLE_GEN
...
Run it in the **Hub's** virtualenv (it has gridstatus + the ERCOT credentials):
    ...
    # or double-click "Refresh Hidalgo Mirasole Wind Data.command"
...
def main() -> int:
    ...
    print(f"Hidalgo Mirasole Wind ({node}) — latest available SCED date: {latest}\n")
