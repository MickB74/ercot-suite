#!/usr/bin/env python3
"""Top up Stafford Solar's cached data to the latest available ERCOT date — one command.
...
Advances both streams the portal settles on, into the shared Data Hub lake:
  ...
  * node price — RT15 settlement-point price at BUZI_SLR_RN
...
BACKFILL_START = dt.date(2025, 10, 1)   # Stafford/Buzios COD 2025-10-01
...
def main() -> int:
    ...
    # Node PRICE (RT15) has no 60-day SCED lag — load it through the live
    # window, not capped at the SCED date. If the price is there, load it.
    price_latest = dt.date.today() - dt.timedelta(days=1)
    print(f"Stafford Solar ({node}) — latest available SCED date: {latest}\n")
