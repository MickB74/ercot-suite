# Stafford Solar — Settlement Portal

A focused, customer-facing web app for a **single asset**: Stafford Solar
(252 MW single-axis PV, Motley County, ERCOT West hub, resource node
`BUZI_SLR_RN`; ERCOT/queue codename "Buzios", legal entity Roaring Springs
Solar LLC, offtaker AdventHealth). It lets the offtaker log in and see, in plain language:
...
- **Projected Bill** — a forward *estimate* (clearly labelled as such, not an
  ...
  typical-meteorological-year shape for Stafford Solar's coordinates/system, rescaled to
...
- The `Ercot_Data_Hub` repo present as a sibling directory (or point
  `HORNET_SOLAR_HUB_ROOT` at it). That repo owns the engine and the Stafford Solar data it
...
# or just double-click "Open Stafford Solar Portal.command"
```
...
collect each month. Top up Stafford Solar's generation **and** node prices to the
...
# or double-click "Refresh Stafford Solar Data.command"
```
