"""Locate the shared ERCOT Data Hub and read Stafford Solar's cached data.
...
and cached for the Stafford Solar node. No live ERCOT fetch happens here, so the portal
loads instantly and works offline.

Resolution order for the Hub root (first that exists wins):
  1. ``$HORNET_SOLAR_HUB_ROOT`` environment variable
...
def _candidate_roots() -> list[Path]:
    roots: list[Path] = []
    env = os.environ.get("HORNET_SOLAR_HUB_ROOT")
    if env:
...
def hub_root() -> Path:
    ...
    raise FileNotFoundError(
        ...
        "and the cached Stafford Solar data). Tried:\n  " + tried +
        "\n\nKeep `Ercot_Data_Hub` as a sibling of this repo, or set the "
        "HORNET_SOLAR_HUB_ROOT environment variable to its path."
    )
...
# --------------------------------------------------------------------------- #
# Data-lake readers — Stafford Solar node generation & node prices (RT15)
# --------------------------------------------------------------------------- #
...
def available_locations() -> tuple[str, ...]:
    ...
    The node (``BUZI_SLR_RN``) always leads; the trading hubs are those with
...
def solar_tmy_hourly(resource_name: str, capacity_kw: float) -> pd.DataFrame | None:
    ...
    Meteorological-Year weather for Stafford Solar's coordinates/system and cached the
