...
def read_price(locs, start, end_excl):
    return _read_stored(pn.PRICE_TEMPLATE, "location", locs, start, end_excl)
    ...
    if want_price:
        with st.spinner("Pulling settlement-point prices (SPP)…"):
            price = npx.fetch_prices(locs, start, end, location_type=location_type,
                                     fetched_at=fetched_at, verbose=False)
...
with st.sidebar:
    ...
    import tzutil
    today = tzutil.now_central().tz_localize(None).normalize()  # ERCOT "today"
    ...
    want_price = st.checkbox("Price (SPP RT15 + DAM)", value=True)
...
for loc in locs:
    ...
    if not price.empty:
        for mkt, dash in (("RT15", "solid"), ("DAM", "dot")):
            p = price[(price["location"] == loc) & (price["market"] == mkt)].sort_values("interval_start")
            if not p.empty:
                fig.add_trace(go.Scatter(x=p["interval_start"], y=p["spp"],
                                         name=f"{loc} — {mkt} $/MWh", mode="lines",
