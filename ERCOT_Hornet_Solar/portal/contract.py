...
# ── the asset ───────────────────────────────────────────────────────────────
# Hornet Solar — ERCOT resource node HRNT_SLR_RN, single SCED unit HRNT_SLR_UNIT1.
...
DEFAULT_CONTRACT = {
    ...
    "strike": 25.0,              # $/MWh — the fixed contract price
    ...
    "offtaker": "Pfizer; VPPA; Oct; Brunswick Corporation also an offtaker",              # full company name of the VPPA buyer
    "developer": "Vesper Energy",             # developer / entity above the project SPV
    "currency": "USD",
...
def settle_location(terms: dict) -> str:
    ...
    ``HRNT_SLR_RN`` or any trading hub such as ``HB_SOUTH``). When it's blank,
...
def eia_plant_id() -> int | None:
    """Hornet Solar's EIA-923 plant id for the SCED cross-check, or None if unmapped.
