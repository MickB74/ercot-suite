"""Locate the shared ERCOT Data Hub and read Hornet Solar's cached data.
...
and cached for the Hornet Solar node. No live ERCOT fetch happens here, so the portal
...
def hub_root() -> Path:
    ...
    raise FileNotFoundError(
        ...
        "and the cached Hornet Solar data). Tried:\n  " + tried +
...
# --------------------------------------------------------------------------- #
# Data-lake readers — Hornet Solar node generation & node prices (RT15)
# --------------------------------------------------------------------------- #
...
def available_locations() -> tuple[str, ...]:
    ...
    The node (``HRNT_SLR_RN``) always leads; the trading hubs are those with
...
def solar_tmy_hourly(resource_name: str, capacity_kw: float) -> pd.DataFrame | None:
    ...
    Meteorological-Year weather for Hornet Solar's coordinates/system and cached the
