"""Run the shared settlement engine for Hornet Solar over a date window.
...
def settle(start_date: dt.date, end_date: dt.date, terms: dict | None = None) -> dict | None:
    """Settle Hornet Solar over [start_date, end_date] inclusive. None if no data."""
    ...
    ref = contract.settle_location(terms)         # node HRNT_SLR_RN or a trading hub
...
def tmy_monthly_mwh(share: float = 1.0) -> pd.Series | None:
    ...
    The "expected" production shape for a representative weather year at Hornet Solar's
    ...
    hourly = hub.solar_tmy_hourly(a["resource_name"], a["capacity_mw"] * 1000.0 * ratio)
...
def calibrate(actual_by_month: pd.Series, tmy_by_month: pd.Series,
    ...
    Partial months (<80% of expected intervals), ramp months (<60% of TMY),
    and months with fewer than *min_years* observations are excluded.
    """
