...
with tab_long:
    ...
    if basis == "Calibrated model":
        cal_factor = st.sidebar.number_input(
            ...
            help=f"Median per-month ratio of metered-to-TMY output ({auto_factor:,.1%}) "
                 f"over {cal['months'] if cal else 0} full-coverage months. "
                 f"Per-month factors are used when available; unobserved months fall back to 1.0 (raw TMY).")
    ...
    if basis == "Calibrated model" and cal:
        st.caption(
            ...
            f"(**{tmy_mwh.sum():,.0f} MWh/yr** at your share) with per-month factors "
            f"(median **{cal_factor:.3f}**) over {cal['months']} full-coverage months, "
            f"degraded {degr:.1%}/yr. Unobserved months use raw TMY (factor 1.0).")
