# Markum Solar — Settlement Portal

A focused, customer-facing web app for a **single asset**: Markum Solar
(161 MW single-axis PV, Bosque County, ERCOT North hub, resource node
`MRKM_SLR_RN`). It lets the offtaker log in and see, in plain language:
...
- **Projected Bill** — a forward *estimate* (clearly labelled as such, not an
  ...
  typical-meteorological-year shape for Markum's coordinates/system, rescaled to
...
- The `Ercot_Data_Hub` repo present as a sibling directory (or point
  `MARKUM_HUB_ROOT` at it). That repo owns the engine and the Markum data it
...
# or just double-click "Open Markum Portal.command"
```
...
collect each month. Top up Markum's generation **and** node prices to the
...
# or double-click "Refresh Markum Data.command"
```
