...
def _forecast_band(hub_name, horizon, ratio_key, asof_iso):
    # Hourly-shaped, year-trended capture when the asset has SCED + price shapes:
    # level anchored to realized, midday belly deepened forward (solar 4%/yr) so
    # forward capture erodes as the duck curve worsens. Falls back to the static
    # seasonal ratio when shapes are unavailable.
...
with tab_long:
    ...
    basis = st.sidebar.radio(
        ...
        help="**Calibrated model** — PVWatts typical-meteorological-year shape, "
             "rescaled so it matches Markum's real metered output. "
    ...
    # Continue the midday-cannibalization trend PAST the engine horizon: the
    # beyond-horizon months repeat a calendar-month shape (capture frozen at the
    # in-horizon level), so multiply them by the shaped capture decline per year
    # (solar only; ratio = shaped_trended ÷ realized base, ≤1 and falling).
    ...
    if use_forecast and band_cal is not None:
        try:
            ...
            for _ym, _r in _shp.items():
                ...
                if _b:
                    _decl[_ym] = _r / _b
        except Exception:  # noqa: BLE001 — trend continuation is best-effort
            _decl = {}
    rows = []
    for i in range(n_months):
        ...
        if band_idx is not None and month_key in band_idx.index:
            ...
            _d = _decl.get(month_key, 1.0)            # continuing capture decline
