...
# Title font stack: Avenir Next (per the deck) → graceful web fallbacks; body is
# Open Sans (the SR body face), loaded from Google Fonts.
...
<style>
  ...
  .azure-foot b {{ color: {SR_DARK_GREY}; }}
</style>
"""
...
def eyebrow(st, text: str) -> None:
    ...
    st.markdown(f'<div class="azure-eyebrow">{text}</div>', unsafe_allow_html=True)
...
def hero(st, title: str, subtitle: str = "") -> None:
    ...
    st.markdown(
        ...
        f'<div class="eyebrow">Azure Sky Wind · Settlement Portal</div>'
...
def footer(st) -> None:
    year = pd_year()
    st.markdown(
        f'<div class="azure-foot">Azure Sky Wind Settlement Portal · prepared by '
        f'<b>Sustainability Roundtable, Inc.</b> · figures derived from '
        f'ERCOT-published 15-minute SCED telemetry and HB_NORTH settlement-point '
        f'prices. Settlement is offtaker-signed: <b>positive = you receive, '
        f'negative = you pay</b>.<br>© {year} Sustainability Roundtable, Inc. · '
        f'Confidential — for use in connection with SR Inc. services only.</div>',
        unsafe_allow_html=True,
    )
