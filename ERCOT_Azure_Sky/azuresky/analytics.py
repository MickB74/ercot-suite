"""Run the shared settlement engine for Azure Sky over a date window.

Thin glue: load the aggregated VORTEX generation + HB_NORTH hub price, then call
...
def settle(start_date: dt.date, end_date: dt.date, terms: dict | None = None) -> dict | None:
    ...
    res = core.settlement.compute_settlement(
        ...
        ref_location=ref,                              # settle at the trading hub
        market="RT15",
        units=a["units"],
        price_floor=floor,
...
def typical_monthly_mwh(share: float = 1.0) -> pd.Series | None:
    """Typical-year MWh by calendar month (1–12) from the cached wind-model run.

    The "expected" production shape for a representative year at Azure Sky's own
    coordinates and turbine fleet, scaled to the offtaker's ``share`` of the
    plant. Returns a Series indexed 1–12, or None if no profile is cached.
    """
    ...
    mwh = ac_kw.groupby(months).sum() / 1000.0        # hourly kW → MWh
    mwh.index.name = "cal_month"
    return (mwh * float(share)).round(3)


def calibrate(actual_by_month: pd.Series, typ_by_month: pd.Series) -> dict:
    """Anchor the typical-year shape to Azure Sky's real metered output.
    ...
    curtailment, true losses). The calibration factor rescales the whole typical
    profile so its total over the *observed* months equals the metered total:

        factor = Σ actual_mwh(observed months) / Σ typ_mwh(same months)

    Returns ``{factor, actual_mwh, typ_mwh, months}`` — ``factor`` is 1.0 (and
    ``months`` 0) when there's no overlap to calibrate against.
    """
    common = actual_by_month.index.intersection(typ_by_month.index)
    a_sum = float(actual_by_month.reindex(common).sum())
    t_sum = float(typ_by_month.reindex(common).sum())
    factor = (a_sum / t_sum) if t_sum > 0 else 1.0
    return {"factor": factor, "actual_mwh": a_sum, "typ_mwh": t_sum,
            "months": int(len(common))}


def sced_monthly_total_mwh(start_date: dt.date, end_date: dt.date) -> pd.Series:
    """Plant-total metered MWh per month (full output, no volume share).
    ...
    share. MWh = Σ mw × 0.25, summed across the four units, grouped by month.
    """
    a = contract.ASSET
    start = pd.Timestamp(start_date)
    end_excl = pd.Timestamp(end_date) + pd.Timedelta(days=1)
    g = hub.generation(a["resource_node"], a["units"], start, end_excl)
    if g.empty:
        return pd.Series(dtype=float)
    g = g.copy()
    g["mwh"] = g["mw"] * 0.25                          # 15-min interval → MWh
...
def reconcile_eia(start_date: dt.date, end_date: dt.date,
    ...
    An independent second source: SCED is ERCOT's telemetry; EIA-923 is the
    plant's own monthly filing. Where they diverge beyond ``tolerance_pct`` the
    metered figure (and any settlement built on it) deserves a second look.

    Returns ``None`` if no EIA plant id is mapped (cross-check disabled), else a
    dict: ``table`` (per-month SCED vs EIA, delta, % and a ``flag``),
    ``plant_id``, ``compared``, ``flagged``, ``tolerance_pct``.
    """
    ...
    if sced.empty:
        return {"table": pd.DataFrame(), "plant_id": plant_id,
                "compared": 0, "flagged": 0, "tolerance_pct": float(tolerance_pct)}
    ...
    return {"table": df, "plant_id": plant_id, "compared": compared,
            "flagged": flagged, "tolerance_pct": float(tolerance_pct)}
