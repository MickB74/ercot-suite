...
from azuresky import analytics, branding, contract, hub, settings, statement  # noqa: E402
...
with st.expander(hdr, expanded=folder is None and not cur):
    new = st.text_input(
        ...
        placeholder=str(Path.home() / "Azure Sky" / "statements"),
...
def _audit_one(raw, sinfo):
    ...
    price = hub.hub_prices(a["hub"], start, end)
    if price.empty:
        raise ValueError("no cached ERCOT price for these dates")
    basis = analytics.pick_time_basis(raw2, mp, price, a["hub"])   # ending vs beginning
    if basis != mp.get("time_basis"):
        mp["time_basis"] = basis
        inv = INV.load_invoice(raw2, mp)
    gen = (hub.generation(a["resource_node"], a["units"], start, end)
           if (net_mode or volume_source == "metered") else None)
    if net_mode:
        return analytics.audit_net_settlement(
            inv, price, terms, location=a["hub"], resource_node=a["resource_node"],
            units=a["units"], gen_df=gen, volume_basis=volume_source, sign=stmt_sign,
            neg_treatment=neg_treatment, neg_floor=neg_floor)
    return INV.reconcile(inv, price_df=price, gen_df=gen, location=a["hub"], market="RT15",
                         resource_node=a["resource_node"], units=a["units"],
...
def _run_batch(items):
    ...
    dl = hub.export_block()
    if dl is not None:
        dl(st, summ, name="azure_sky_portfolio_audit",
           title="Azure Sky Wind — portfolio invoice audit",
...
def _pdf_audit(src, name):
    ...
    if not all(k in summ for k in need):
        ...
        return
    r = analytics.audit_summary(summ, terms, resource_node=a["resource_node"], units=a["units"])
    ...
    if "eia_volume" in r:
        ...
        st.markdown((f"- {'✅' if abs(vp) < 2 else '⚠️'} Billed volume **{r['volume_mwh']:,.0f} MWh** "
                     f"vs EIA-923 wind (your {contract.offtake_label(terms)}) "
        ...
        st.markdown("- ℹ️ EIA-923 volume check unavailable for this month (not yet published, "
                    "or before Azure Sky Wind's 2023 EIA reporting).")
...
price_df = hub.hub_prices(a["hub"], start, end_excl)
gen_df = (hub.generation(a["resource_node"], a["units"], start, end_excl)
          if (net_mode or volume_source == "metered") else None)

if price_df.empty:
    st.error("No cached ERCOT price covers this statement's dates for HB_NORTH — the "
             "internal Data Hub may need to pull that window first.")
    st.stop()

if _basis_choice == "auto-detect":
    _b = analytics.pick_time_basis(raw, m, price_df, a["hub"])
    if _b != m["time_basis"]:
        m["time_basis"] = _b
        inv = INV.load_invoice(raw, m)
...
if net_mode:
    res = analytics.audit_net_settlement(
        inv, price_df, terms, location=a["hub"], resource_node=a["resource_node"],
        units=a["units"], gen_df=gen_df, volume_basis=volume_source, sign=stmt_sign,
        neg_treatment=neg_treatment, neg_floor=neg_floor)
    s = res["summary"]
    ...
    if s.get("eia_volume_total"):
        ...
        st.caption(f"🔢 **EIA-923 volume check** ({s.get('eia_months', 0)} month(s)) — bill "
                   f"**{eb:,.1f} MWh** vs EIA-923 wind net generation (your "
    ...
    if download_block is not None:
        download_block(
            st, intervals, name=f"azure_sky_net_settlement_audit_{pd.Timestamp(lo).date()}",
            title="Azure Sky Wind — net settlement audit",
            meta={"Asset": a["project_name"], "Hub": a["hub"], "Statement": src_label or "",
        ...
        st.download_button("⬇ Download net-settlement audit CSV",
                           ...
                           file_name="azure_sky_net_settlement_audit.csv", mime="text/csv")
    branding.footer(st)
    st.stop()

# ── gross energy-invoice reconcile (amount = price × volume) ────────────────
res = INV.reconcile(inv, price_df=price_df, gen_df=gen_df, location=a["hub"], market="RT15",
                    resource_node=a["resource_node"], units=a["units"],
                    volume_source=volume_source)
s = res["summary"]
intervals = res["intervals"]
...
if download_block is not None:
    download_block(
        st, intervals, name=f"azure_sky_invoice_audit_{pd.Timestamp(lo).date()}",
        title="Azure Sky Wind — invoice audit",
        meta={"Asset": a["project_name"], "Hub": a["hub"], "Statement": src_label or "",
    ...
    st.download_button("⬇ Download reconciliation CSV",
                       intervals.to_csv(index=False).encode("utf-8"),
                       file_name="azure_sky_invoice_audit.csv", mime="text/csv")
