...
so this page *estimates* it: expected generation (from a weather-typical wind
model anchored to Azure Sky's real metered output, or its own historical shape)
× a forward market price you can set, settled against the strike. It's a planning
figure, not an invoice — clearly labelled as such.
"""
...
st.info("📌 **Estimate only.** Generation is modelled — by default from a "
        "weather-calibrated wind model (typical year, anchored to Azure Sky's real "
        "metered output) — and the market price is your forward assumption. Switch "
...
# Typical-year shape from the cached wind-model run (None if not cached yet).
...
if typ_mwh is None:
    ...
    st.sidebar.caption("No typical-year wind profile is cached for this plant yet, "
                       "so only the historical shape is available. Run the Hub's "
                       "plant-value step to enable the calibrated model.")
basis = st.sidebar.radio(
    "Generation basis", bases, index=0,
    help="**Calibrated model** — weather-typical wind profile rescaled so it "
         "matches Azure Sky's real metered output (captures actual availability, "
         "curtailment and losses). **Physical model** — raw typical year, no "
         "calibration. **Historical shape** — mean of each calendar month across "
         "the metered history.")
...
if basis == "Calibrated model" and cal:
    ...
    st.caption(f"Generation basis: **physical model** — raw typical-year wind "
               f"profile (**{typ_mwh.sum():,.0f} MWh/yr** at your share), uncalibrated.")
else:
