...
if st.button("⬇️ Refresh now", type="primary",
    ...
    # The generation + AZURE_RN price pulls use gridstatus + ERCOT credentials,
    # which live in the Data Hub's virtualenv — NOT this display portal's venv.
    # So we run the asset's own proven refresh.py under the Hub venv as a streamed
    # subprocess (same reason the hub-price section shells out via orchestrate),
    # rather than importing the gridstatus-backed pullers in-process.
    if do_gen or do_price:
        ...
        if not hub_py.exists():
            st.error(f"Data-pull venv not found at `{hub_py}`. The generation + node-price "
                     ...
                     'You can also double-click "Refresh Azure Sky Data.command".')
        elif not refresh_py.exists():
            st.error(f"refresh.py not found at `{refresh_py}`.")
        else:
            cmd = [str(hub_py), str(refresh_py)] + (["--full"] if full else [])
            with st.status("Refreshing generation + AZURE_RN price via the Data Hub "
                           "venv (archive pulls can take a few minutes)…",
