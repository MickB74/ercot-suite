# Aguayo Wind — Settlement Portal

A focused, customer-facing web app for a **single asset**: Aguayo Wind
(161 MW single-axis PV, Bosque County, ERCOT North hub, resource node
`AGUAYO_UNIT1`). It lets the offtaker log in and see, in plain language:
...
- **Projected Bill** — a forward *estimate* (clearly labelled as such, not an
  ...
  typical-meteorological-year shape for Aguayo Wind's coordinates/system, rescaled to
...
- The `Ercot_Data_Hub` repo present as a sibling directory (or point
  `AGUAYO_WIND_HUB_ROOT` at it). That repo owns the engine and the Aguayo Wind data it
...
# or just double-click "Open Aguayo Wind Portal.command"
```
...
collect each month. Top up Aguayo Wind's generation **and** node prices to the
...
# or double-click "Refresh Aguayo Wind Data.command"
```
