"""Locate the shared ERCOT Data Hub and read Aguayo Wind's cached data.
...
and cached for the Aguayo Wind node. No live ERCOT fetch happens here, so the portal
loads instantly and works offline.

Resolution order for the Hub root (first that exists wins):
  1. ``$AGUAYO_WIND_HUB_ROOT`` environment variable
...
def _candidate_roots() -> list[Path]:
    roots: list[Path] = []
    env = os.environ.get("AGUAYO_WIND_HUB_ROOT")
    if env:
...
def hub_root() -> Path:
    ...
    raise FileNotFoundError(
        ...
        "and the cached Aguayo Wind data). Tried:\n  " + tried +
        "\n\nKeep `Ercot_Data_Hub` as a sibling of this repo, or set the "
        "AGUAYO_WIND_HUB_ROOT environment variable to its path."
    )
...
# --------------------------------------------------------------------------- #
# Data-lake readers — Aguayo Wind node generation & node prices (RT15)
# --------------------------------------------------------------------------- #
...
def available_locations() -> tuple[str, ...]:
    ...
    The node (``AGUAYO_UNIT1``) always leads; the trading hubs are those with
...
def solar_tmy_hourly(resource_name: str, capacity_kw: float) -> pd.DataFrame | None:
    ...
    Meteorological-Year weather for Aguayo Wind's coordinates/system and cached the
