"""Run the shared settlement engine for Aguayo Wind over a date window.
...
def settle(start_date: dt.date, end_date: dt.date, terms: dict | None = None) -> dict | None:
    """Settle Aguayo Wind over [start_date, end_date] inclusive. None if no data."""
    ...
    ref = contract.settle_location(terms)         # node AGUAYO_UNIT1 or a trading hub
...
def tmy_monthly_mwh(share: float = 1.0) -> pd.Series | None:
    ...
    The "expected" production shape for a representative weather year at Aguayo Wind's
