...
with st.expander(hdr, expanded=folder is None and not cur):
    new = st.text_input(
        ...
        placeholder=str(Path.home() / "Aguayo Wind" / "statements"),
...
def _run_batch(items):
    ...
    if dl is not None:
        dl(st, summ, name="portal_portfolio_audit",
           title="Aguayo Wind — portfolio invoice audit",
...
if net_mode:
    ...
    if download_block is not None:
        download_block(
            ...
            title="Aguayo Wind — net settlement audit",
...
if download_block is not None:
    download_block(
        ...
        title="Aguayo Wind — invoice audit",
        meta={"Asset": a["project_name"], "Settles at": loc, "Statement": src_label or "",
