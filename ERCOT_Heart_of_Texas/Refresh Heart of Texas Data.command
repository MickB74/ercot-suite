#!/bin/bash
# Double-click to top up Heart of Texas Wind's generation + node prices to the
# latest ERCOT date. Runs refresh.py with the Hub's virtualenv (it has the
# ERCOT-pull deps and the API credentials).
cd "$(dirname "$0")" || exit 1

# Find the Hub's python: env override, sibling repo, then the usual location.
CANDIDATES=(
  "${HOT_WIND_HUB_ROOT:-}/.venv/bin/python"
...
if [ -z "$HUB_PY" ]; then
  ...
  echo "Set HOT_WIND_HUB_ROOT to the Hub repo, or keep it as a sibling directory."
