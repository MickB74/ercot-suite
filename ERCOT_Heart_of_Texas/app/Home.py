"""Heart of Texas Wind Settlement Portal — router / entry point.

A focused, customer-facing app for one asset (Heart of Texas Wind). It reuses
the shared ERCOT engine and cached data from the sibling ``Ercot_Data_Hub``
repo; see :mod:`hotwind.hub`. This file owns ``st.set_page_config`` for the
whole app — the page scripts must not call it again.
...
# Make the ``hotwind`` package importable however the app is launched.
...
from hotwind import contract  # noqa: E402
...
nav = st.navigation({
    ...
    "Audit": [
        ...
        st.Page("pages/8_Basis_Differential.py", title="Basis Differential", icon="⚖️"),
        st.Page("pages/7_Hub_vs_Node.py", title="Hub vs Node", icon="📡"),
    ],
    "Analysis": [
        st.Page("pages/9_Generation_Analysis.py", title="Generation Analysis", icon="⚡"),
        st.Page("pages/10_Capture_Analysis.py", title="Capture Analysis", icon="🎯"),
    ],
    "Data": [
