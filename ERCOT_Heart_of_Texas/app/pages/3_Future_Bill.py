...
Two views:
  ...
  • Long range (TMY / history) — the original whole-month projection out to
    12 months, using wind typical-year model or the historical metered shape.
"""
...
from hotwind import analytics, branding, contract, hub  # noqa: E402
...
cal = (analytics.calibrate(hist_mwh, typ_mwh, monthly_intervals=_interval_counts,
                           monthly_counts=hist_counts)
       if typ_mwh is not None else None)
...
def _capture_ratios(hub_name, win_start_iso, win_end_iso, monthly_key):
    ...
    d = price_forecast.capture_to_hub_monthly(md, h, price_col=price_col,
                                               ...
                                               node=a["resource_node"], spring_trend_pct=0.0, fleet_fallback=price_forecast.fleet_capture_ratios(hub_name))
...
with tab_long:
    ...
    basis = st.sidebar.radio(
        ...
        help="**Calibrated model** — weather-typical wind profile shape, "
             "rescaled so it matches Heart of Texas Wind's real metered output. "
             "**Physical model** — raw typical year, uncalibrated. "
    ...
    if basis == "Calibrated model" and cal:
        ...
        st.caption(
            ...
            f"(**{typ_mwh.sum():,.0f} MWh/yr** at your share). "
        ...
        st.caption(f"**Physical model** — raw weather-typical wind profile "
