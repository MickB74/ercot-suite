...
with st.expander(hdr, expanded=folder is None and not cur):
    new = st.text_input(
        ...
        placeholder=str(Path.home() / "Heart of Texas Wind" / "statements"),
...
def _run_batch(items):
    ...
    if dl is not None:
        dl(st, summ, name="portal_portfolio_audit",
           title="Heart of Texas Wind — portfolio invoice audit",
...
if net_mode:
    ...
    if download_block is not None:
        download_block(
            ...
            title="Heart of Texas Wind — net settlement audit",
...
if download_block is not None:
    download_block(
        ...
        title="Heart of Texas Wind — invoice audit",
        meta={"Asset": a["project_name"], "Settles at": loc, "Statement": src_label or "",
