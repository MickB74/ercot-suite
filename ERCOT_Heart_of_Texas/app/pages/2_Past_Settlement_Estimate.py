...
st.success(
    f"Over **{cov_min} → {cov_max}**, Heart of Texas Wind produced **{mwh:,.0f} MWh**, captured at "
...
if len(monthly) > 1:
    ...
    if download_block_m is not None:
        download_block_m(
            ...
            title=f"Heart of Texas Wind — monthly settlement {start_d} → {end_d}",
...
if recon is None:
    with st.expander("Cross-check vs EIA-923 (independent source)"):
        st.caption(
            ...
            "looks off. Heart of Texas Wind isn't mapped to an EIA plant id yet (there's no "
...
if download_block is not None:
    download_block(
        ...
        title=f"Heart of Texas Wind settlement — {start_d} → {end_d}",
        meta={"Asset": a["project_name"], "Settles at": loc,
              "Structure": terms["structure"], "Strike": f"${terms['strike']:,.2f}/MWh",
