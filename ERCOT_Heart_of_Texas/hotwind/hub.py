"""Locate the shared ERCOT Data Hub and read Heart of Texas Wind's cached data.
...
and cached for the Heart of Texas Wind node. No live ERCOT fetch happens here,
so the portal loads instantly and works offline.

Resolution order for the Hub root (first that exists wins):
  1. ``$HOT_WIND_HUB_ROOT`` environment variable
...
def _candidate_roots() -> list[Path]:
    roots: list[Path] = []
    env = os.environ.get("HOT_WIND_HUB_ROOT")
    if env:
...
def hub_root() -> Path:
    ...
    raise FileNotFoundError(
        ...
        "and the cached Heart of Texas Wind data). Tried:\n  " + tried +
        "\n\nKeep `Ercot_Data_Hub` as a sibling of this repo, or set the "
        "HOT_WIND_HUB_ROOT environment variable to its path."
    )
...
# --------------------------------------------------------------------------- #
# Data-lake readers — Heart of Texas Wind node generation & node prices (RT15)
# --------------------------------------------------------------------------- #
...
def eia_monthly_netgen(plant_id: int, start_year: int, end_year: int,
    ...
    wind) restricts to one prime mover — needed for co-located plants under one
